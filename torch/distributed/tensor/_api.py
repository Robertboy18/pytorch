    ...
    def redistribute(
        ...
        *,
        ...
        6. ``Shard(dim)`` -> ``Partial("sum")``: local zero-filling. This
           explicit-only conversion materializes a logical-shape local tensor
           on every rank and does not communicate.

        Of conversions to ``Partial``, this public API supports only
        ``Shard(dim)`` -> ``Partial("sum")``. Its disjoint, zero-filled
        contributions reconstruct the logical tensor without scaling. Internal
        ``Replicate()`` -> ``Partial("sum")`` divides values by the mesh size and
        is not exposed because it may introduce floating-point error.
